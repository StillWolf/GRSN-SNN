    ...
    def forward(self, x, states=None):
        ...
        for t in range(T):
            ...
            for k in range(self.time_step):
                current_state, spike_list[0] = self.cells[0](x[t], current_state)
            new_states_list[0] = current_state
            for i in range(1, self.num_layers):
                y = new_states_list[i - 1]
                current_state = states_list[i]
                for k in range(self.time_step):
                    current_state, spike_list[i] = self.cells[i](y, current_state)
                new_states_list[i] = current_state
