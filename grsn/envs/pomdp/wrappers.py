...
class POMDPWrapper(gym.Wrapper):
    def __init__(self, env_fn, partially_obs_dims: list):
        env = env_fn()
        super().__init__(env)
    ...
    def reset(self, **kwargs):
        ...
        # Handle new gym API (v0.26+) that returns (obs, info)
        if isinstance(result, tuple):
            state, info = result
            return self.get_obs(state)
        else:
            return self.get_obs(result)
    ...
    def step(self, action):
        ...
        # Handle new gym API (v0.26+) that returns (obs, reward, terminated, truncated, info)
        if len(result) == 5:
        ...
        # Return 5 values to match new gym API (v0.26+)
        if len(result) == 5:
