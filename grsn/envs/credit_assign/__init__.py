...
from grsn.envs.credit_assign.key_to_door import key_to_door
...
for runs in [1, 2, 5, 10, 20, 40]:
    register(
        f"Catch-{runs}-v0",
        entry_point="grsn.envs.credit_assign.catch:DelayedCatch",
        kwargs=dict(
            delay=delay_fn(runs),
            flatten_img=True,
...
register(
    "KeytoDoor-SR-v0",
    entry_point="grsn.envs.credit_assign.key_to_door.tvt_wrapper:KeyToDoor",
    kwargs=dict(
        flatten_img=True,
        one_hot_actions=False,
        apple_reward=1.0,
